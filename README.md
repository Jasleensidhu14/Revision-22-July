# Revision-22-July
MY SQL QUERIES 
//////REVISION - 17 JULY 2022, 22 and 23 JULY 2022/////
show databases- this shows all the created databases you have till now.
...
//COUNT- counts all the total records added//
select age,loan, job from bank_detail
//SPecific Columns if you want to see// 
select `default` from bank_detail 
select * from bank_detail limit 10 
select age from bank_detail where age> 30 
select * from bank_detail where education = 'Unknown' and marital ='Single' 
select distinct job from bank_detail
select * from bank_detail where (marital ='single'or education = 'unknown' ) and balance < 500  
select count(*) from bank_detail 
select distinct balance from bank_detail
